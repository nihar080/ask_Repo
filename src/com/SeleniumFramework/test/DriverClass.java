...
public class DriverClass extends FunctionalLibrary
{
	...
	public void newTest() throws Exception
	{
		if (excelFileUtil.driverInstance.equalsIgnoreCase("WebDriver"))
		{
			...
			reportzip="SeleniumFramework\\TestExecutionZip_Reports\\" +excelFileUtil.result_backup_name + "_"+ zipdate + ".zip";	
	...
	/**
	 * importData method gets the location/Path of Test Suite, Test Modules, Element Collection files
	 * @param testUtility testUtility contains the path of provider excelsheet where paths of all above required files/folders are stored
	 */
...
private void executeForChrome() throws Exception
{
    	...
    	if(excelFileUtil.environment.equalsIgnoreCase("Desktop_Web"))
    	{
    		...
    		oldTab = driver.getWindowHandle();
    		 ...
    		 excelFileUtil.tmpBrowserVer = rv;
    		 testExecutor.testSuite(moduleName, reportLib, seleniumHandler,rv); //Calls testSuite method with new user agent each time
    		 PREVIOUS_TEST_CASE = "Before Test Execution";
    		 LOG_VAR=1;
    		 TEST_STEP_COUNT = 1;
    		 failedStep = " ";
